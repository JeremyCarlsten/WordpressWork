...
<?php do_action('woocommerce_before_add_to_cart_form'); ?>
    ...
    <form class="cart uou-form" method="post" enctype='multipart/form-data'>
        ...
        <div id="uou-bookings-booking-form">
            ...
            <select class="my_select_box" multiple data-placeholder="Select Your Options" name="resoruce_select">
                ...
                foreach ($booking_resource_meta as $meta) {
                    ...
                    <option
                        value='{"resource_name":"<?php echo $title; ?>","cost":"<?php echo $resource_value; ?>"}'><?php echo $title . ' : &nbsp; &nbsp; ' . get_woocommerce_currency_symbol() . $resource_value; ?></option>
            ...
            <select id="person_cost_dropdown" class="my_select_box" name="person_select"
                ...
                if (is_array($booking_meta)) {
                    foreach ($booking_meta as $meta) {
                        ...
                        if ($person_check == 'uou_person') {
                            ...
                            <option
                                value='{"person_no":"<?php echo $person_no; ?>","cost":"<?php echo $person_cost; ?>"}'><?php echo 'No of person ( ' . $person_no . ' ) - Additional Cost :   &nbsp; &nbsp;' . get_woocommerce_currency_symbol() . $person_cost; ?></option>
            ...
            <p id="check-in-wrapper" class="form-row">
				<span class="calendar-input input-left" title="Departure">
					<input type="text" class="check-in" name="check-in" name="departure"
                           placeholder="<?php _e('Arrival', 'uou-bookings') ?>">
					<i class="fa fa-calendar check-in"></i>
				</span>
            </p>

            <p id="check-out-wrapper" class="calendar-input">
                <input type="text" class="check-out" name="check-out" value=""
                       placeholder="<?php _e('Departure', 'uou-bookings') ?>">
        ...
        <h1><?php _e('Total Booking Cost : &nbsp;', 'uou-bookings') ?><?php echo get_woocommerce_currency_symbol(); ?>
            <span class="total_cost"></span></h1>
