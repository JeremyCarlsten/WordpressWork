...
jQuery(function ($) {
    ...
    $('.cuztom').on('click', '.js-cuztom-remove-sortable', function () {
        ...
        if (fields == 2) {
            $('.js-cuztom-sortable-item', wrap).find('.js-cuztom-remove-sortable').remove();
        }
    });
    ...
    $('.cuztom').on('click', '.js-cuztom-add-sortable', function () {
        ...
        if (is_bundle) {
            new_item.find('tr').each(function () {
                ...
                cuztom_input.attr('name',function (i, val) {
                    return val.replace(/\[(\d+)\]/, function (match, n) {
                        return '[' + ( Number(n) + 1 ) + ']';
                    });
                }).attr('id',function (i, val) {
                        return val.replace(/\_(\d+)/, function (match, n) {
                            return '_' + ( Number(n) + 1 );
                        })
        ...
        $('> .js-cuztom-sortable-item', wrap).each(function (index, item) {
            if ($('> .js-cuztom-handle-sortable', item).length == 0) {
                $(item).prepend(handle);
            }
            if ($('> .js-cuztom-remove-sortable', item).length == 0) {
                $(item).append(remover);
            }
        });
    ...
    $('.cuztom-td').on('click', '.js-cuztom-ajax-save', function () {
        ...
        $.post(Cuztom.ajax_url, data, function (r) {
            ...
            input.animate({ borderColor: '#60b334' }, 200, function () {
                input.animate({ borderColor: border_color });
            });
        });
