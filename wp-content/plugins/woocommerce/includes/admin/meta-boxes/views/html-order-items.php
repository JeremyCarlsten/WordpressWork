...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
    ...
    <table class="wc-order-totals">
        <tr>
            <td class="label"><?php _e('Discount', 'woocommerce'); ?> <span class="tips"
                                                                            data-tip="<?php _e('This is the total discount. Discounts are defined per line item.', 'woocommerce'); ?>">[?]</span>:
            </td>
            <td class="total">
        ...
        <tr>
            <td class="label"><?php _e('Shipping', 'woocommerce'); ?> <span class="tips"
                                                                            data-tip="<?php _e('This is the shipping and handling total costs for the order.', 'woocommerce'); ?>">[?]</span>:
            </td>
        ...
        <tr>
            ...
            <td class="total refunded-total">
                -<?php echo wc_price($order->get_total_refunded(), array('currency' => $order->get_order_currency())); ?></td>
            <td width="1%"></td>
        </tr>
...
<div class="wc-order-data-row wc-order-bulk-actions">
    <p class="bulk-actions">
        ...
        <button type="button" class="button do_bulk_action wc-reload" title="<?php _e('Apply', 'woocommerce'); ?>">
    ...
    <p class="add-items">
        ...
        <?php else : ?>
            <span class="description tips"
                  data-tip="<?php esc_attr_e('To edit this order change the status back to "Pending"', 'woocommerce'); ?>"><?php _e('This order has been paid for and is no longer editable', 'woocommerce'); ?></span>
        <?php endif; ?>
        ...
        <?php if ($order->is_editable()) : ?>
            <button type="button"
                    class="button button-primary calculate-tax-action"><?php _e('Calculate Taxes', 'woocommerce'); ?></button>
            <button type="button"
                    class="button button-primary calculate-action"><?php _e('Calculate Total', 'woocommerce'); ?></button>
        <?php endif; ?>
    </p>
</div>
    ...
    <div class="wc-order-data-row wc-order-refund-items" style="display: none;">
        <table class="wc-order-totals">
            <tr style="display:none;">
                <td class="label"><label
                        for="restock_refunded_items"><?php _e('Restock refunded items', 'woocommerce'); ?>:</label></td>
            ...
            <tr>
                <td class="label"><label for="refund_reason"><?php _e('Reason for refund (optional)', 'woocommerce'); ?>
                        :</label></td>
                <td class="total">
        ...
        <div class="refund-actions">
            ...
            <button type="button" class="button button-primary do-manual-refund tips"
                    data-tip="<?php esc_attr_e('You will need to manually issue a refund through your payment gateway after using this.', 'woocommerce'); ?>"><?php printf(_x('Refund %s manually', 'Refund $amount manually', 'woocommerce'), $refund_amount); ?></button>
            <button type="button" class="button cancel-action"><?php _e('Cancel', 'woocommerce'); ?></button>
            <div class="clear"></div>
        </div>
    </div>
        ...
        <div class="wc-backbone-modal-content">
            <section class="wc-backbone-modal-main" role="main">
                <header class="wc-backbone-modal-header">
                    <a class="modal-close modal-close-link" href="#"><span class="close-icon"><span
            ...
            <section class="wc-backbone-modal-main" role="main">
                ...
                <article>
                    <form action="" method="post">
                        ...
                        <p>
                            <label
                                for="manual_tax_rate_id"><?php _e( 'Or, enter tax rate ID:', 'woocommerce' ); ?></label><br/>
                            <input type="number" name="manual_tax_rate_id" id="manual_tax_rate_id" step="1"
                                   placeholder="<?php _e( 'Optional', 'woocommerce' ); ?>"/>
                        </p>
						<?php endif; ?>
					</form>
                </article>
